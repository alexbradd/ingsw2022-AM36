...
public class IslandTest {
    ...
    void setUp() {
        players = List.of(
                new Player("Napoleon", 1, 10, TowerColor.BLACK),
                new Player("Cesar", 1, 10, TowerColor.WHITE));
        i = new Island(0);
    }
    ...
    void testUpdateStudents() {
        ...
        for (int i = 0; i < 10; i++) l.add(new Student(p));

        i = i.updateStudents(c -> {
            assertTrue(c.getStudents().isEmpty());
            for (Student s : l) c = c.add(s);
            return c;
        });
        i.updateStudents(c -> {
            assertFalse(c.getStudents().isEmpty());
            c.getStudents().forEach(s -> assertTrue(l.contains(s)));
            return c;
        });
        i = i.updateStudents(c -> {
            for (Student s : l) {
                try {
                    c = c.remove(s.getColor(), s1 -> assertTrue(l.contains(s1)));
                } catch (EmptyStudentContainerException e) {
                    assertFalse(false);
                }
            }
            return c;
    ...
    void testMergeContainers() {
        ...
        i = i.updateTowers(t -> List.of(new Tower(TowerColor.BLACK, players.get(0))))
                .updateStudents(c -> {
                    for (int i = 0; i < 10; i++) c = c.add(new Student(p1));
                    return c;
                });
        Island other = new Island(1)
                .updateTowers(t -> List.of(new Tower(TowerColor.BLACK, players.get(0))))
                .updateStudents(c -> {
                    for (int i = 0; i < 10; i++) c = c.add(new Student(p2));
                    return c;
                });
